...
function AddPlantForm({ onPlantAdded, onPlantUpdated, onClose, plantToEdit }) {
    ...
    const [formData, setFormData] = useState({
        ...
        sunlight:
            ...
            (Array.isArray(plantToEdit?.sunlight)
                ? plantToEdit.sunlight[0]
                : ""),
        watering_frequency:
            plantToEdit?.watering_frequency ||
            (plantToEdit?.watering_interval
                ? plantToEdit.watering_interval.unit === "weeks"
                    ? ((plantToEdit.watering_interval.min +
                          plantToEdit.watering_interval.max) /
                          2) *
                      7
                    : (plantToEdit.watering_interval.min +
                          plantToEdit.watering_interval.max) /
    ...
    function handlePlantSelect(plant) {
        ...
        // Set photo from API if available
        // User can override by uploading their own photo
        if (plant.default_image) {
            // Try different possible image fields from the API
            const apiImage =
